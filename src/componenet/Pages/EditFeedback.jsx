...
const EditFeedback = () => {
    const { setCurrentVisibleSuggestions,plannedFeatures,liveFeatures, inProgressFeatures,productRequests, setProductRequests, currentSuggestionId, suggestions, setInProgressFeatures, setplannedFeatures, setLiveFeatures, setSuggestion } = useContext(AppContext)
    ...
    const handleFeedackDeleted = (e) => {
       ...
       else if (feedbackStatus === "planned") {
            setplannedFeatures(prv => prv.filter(item => item.id !== id))
        }
        else if (feedbackStatus === "live") {
            setLiveFeatures(prv => prv.filter(item => item.id !== id))
        }
        else  setInProgressFeatures(prv => prv.filter(item => item.id !== id))
        navigate('/')
