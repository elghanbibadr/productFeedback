...
const RoadpMapItems = (props) => {
        return (
          ...
          {props.roadmapItem.map(({ id, description,category, title, upvotes, comments }, index)=>{
              return   <SuggestionCard status='planned' isSuggestionOfRoadmapPage={true}  key={index} id={id} upvotes={upvotes} category={category} commentsLength={comments ? comments.length : 0} description={description} title={title}   />
